...
server <- function(input, output, session) {
  ...
  observeEvent(input$showMethodology, {
    showModal(modalDialog(
      title = "Methodology",
      tags$p("The trip-length distributions from household travel survey was data was processed using the following steps:"),
      ...
      tags$p("A jupyter notebook was used to calculate a histogram (percentage) distribution and cumulative distribution for each combination of bin size and key dimension. The notebook can be found in this repo: https://github.com/WFRCAnalytics/Resources/tree/master/R-Shiny/HTS-Trip-Lengths/dataprep"),
...
WITH RECURSIVE NumberSeries AS (
  SELECT 0 AS num
  UNION ALL
  SELECT num + 5 FROM NumberSeries WHERE num < 1000
)

-- Half-mile bin sizes up to 100
SELECT 0.5 AS binSize, num / 10.0 AS binStart FROM NumberSeries WHERE MOD(num, 5) = 0 AND num / 10.0 < 100

UNION ALL

-- 1-mile bin sizes up to 100
SELECT 1.0 AS binSize, num / 10.0 AS binStart FROM NumberSeries WHERE MOD(num, 10) = 0 AND num / 10.0 < 100

UNION ALL

-- 5 mile bin sizes up to 100
SELECT 5.0 AS binSize, num / 10.0 AS binStart FROM NumberSeries WHERE MOD(num, 50) = 0 AND num / 10.0 < 100

ORDER BY binSize, binStart
  </code>
</pre>
        ')
      ),
...
observe({
  ...
  output$dataPlot <- renderPlot({
    ...
    filtered_data <- data %>%
      filter(groupSampleSegmentLabel == input$groupSampleSegmentLabel,
  ...
  output$selectedInputs <- renderText({
    paste("\n",
          "Current Filter:",
          paste0("\u00A0\u00A0\u00A0Sample Segment Group: ", labelsSampleSegmentGroups$label[labelsSampleSegmentGroups$value == input$groupSampleSegmentLabel]),
          paste0("\u00A0\u00A0\u00A0Broad Mode: ", labelsModeTypeBroad$label[labelsModeTypeBroad$value == input$groupModeTypeBroad]),
