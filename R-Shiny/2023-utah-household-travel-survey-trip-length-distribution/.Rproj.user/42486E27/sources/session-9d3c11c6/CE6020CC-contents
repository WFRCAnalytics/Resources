...
server <- function(input, output, session) {
  ...
  observeEvent(input$showMethodology, {
    showModal(modalDialog(
      ...
      tags$p("A jupyter notebook was used to calculate a histogram (percentage) distribution and cumulative distribution for each combination of bin size and key dimension. The notebook can be found in this repo: https://github.com/WFRCAnalytics/HTS-Trip-Length-R-Shiny"),
      tags$h4("QUERY 1"),
